...
struct MapView: View {
    ...
    var body: some View {
       ...
       // Map(coordinateRegion: $region)
        ...
        //MARK: -2o Advanced Map
        Map(coordinateRegion: $region, annotationItems: locations, annotationContent: {
            item in
            //(A) PINN: OLD STYLE (always static)
        ...
        .overlay(HStack (alignment: .center, spacing: 12){
            Image("compass").resizable().scaledToFill().frame(width: 48, height: 48, alignment: .center)
            VStack(alignment: .leading, spacing: 3){
                ...
                HStack{
                    Text("Longitude:").font(.footnote).fontWeight(.bold).foregroundColor(.accentColor)
                    Spacer()
                    Text("\(region.center.longitude)").font(.footnote).foregroundColor(.white)
                }//:Hstack
            }//:VSTACK
        }//:HSTACK
            .padding(.vertical, 12).padding(.horizontal, 16).background(
